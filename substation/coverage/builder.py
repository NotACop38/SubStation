...
def _render_tactic_coverage(detections: list[Detection]) -> list[str]:
    ...
    lines = [
        ...
        "This counts content mappings, not measured detection effectiveness or complete "
        "coverage of a tactic.",
        "",
    ...
    for tid, name in _ICS_TACTICS:
        ...
        if ids:
            ...
            lines.append(f"| {name} | {tid} | — | no mapping |")
    lines.append("")
    return lines
...
def render_markdown(detections: list[Detection]) -> str:
    ...
    lines = [
        ...
        "Tier 1 = Sigma-over-JSON (Python packages); "
        ...
        "",
        "`validated` means the synthetic Tier-1 contract passes; `tier2` names the "
        "required engine. All rules are experimental. Independent site data and "
        "sensor/backend qualification remain necessary.",
        "",
        "## Detections",
        "",
...
def render_navigator_layer(detections: list[Detection]) -> str:
    ...
    layer = {
        ...
        "description": (
            ...
            "technique. Scores are content counts, not measured detection effectiveness."
        ),
