"""Shared synthetic-TCP-stream scaffolding for the per-protocol PCAP emitters.

Every protocol emitter frames its application PDUs on the same synthetic but
well-formed TCP stream: a SYN handshake just before the first message, one
...
payload may be raw ``bytes`` (hand-built DNP3/S7 frames) or a scapy layer
(Modbus ADUs); bytes are wrapped in ``Raw`` for the frame.

Everything is deterministic (blake2b-derived ISNs and MACs): identical input
yields a byte-identical PCAP. Writing uses scapy's ``PcapWriter`` (ordinary
file I/O); no socket is ever opened — see :mod:`substation.emit.guard`.
"""
...
class FlowEvent(Protocol):
    """The connection-envelope fields every protocol's event type shares."""
...
class _TcpFlow:
    ...
    def build(self, events: list[E], payload_for: Callable[[E], Any]) -> list[Any]:
        start = events[0].ts
        end = events[-1].ts
        # Three-way handshake just before the first protocol segment.
        ...
        # One PSH/ACK per event, carrying the PDU; acks piggyback on data.
        for event in events:
            self._emit(
...
def write_pcap(events: Iterable[E], path: str | Path, payload_for: Callable[[E], Any]) -> int:
    """Write ``events`` as a TCP capture to ``path``; return the packet count.

    Events are grouped into per-connection TCP flows (by uid), each framed with
    a handshake, data segments and teardown, then all packets are written in
    timestamp order. ``payload_for`` maps one event to its application PDU. An
    empty event list still yields a valid (header-only) PCAP.
    """
    by_connection: dict[str, list[E]] = {}
    for event in events:
