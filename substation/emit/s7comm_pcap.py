...
shared :mod:`substation.emit._tcp` scaffold. Output is deterministic: identical
input yields byte-identical PCAP. Writing uses scapy's ``PcapWriter`` (ordinary
...
def write_pcap(events: Iterable[S7Event], path: str | Path) -> int:
    ...
    """
    return _tcp.write_pcap(events, path, _pdu_bytes)
