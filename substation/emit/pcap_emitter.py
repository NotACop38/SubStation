...
this module's empirical checks — never invented from memory). The synthetic TCP
framing (handshake, PSH/ACK segments, teardown) is the shared
:mod:`substation.emit._tcp` scaffold. Output is deterministic: identical input
yields byte-identical PCAP. Writing uses scapy's ``PcapWriter`` (ordinary file
I/O); no socket is ever opened — see :mod:`substation.emit.guard`.
"""
...
def write_pcap(events: Iterable[ModbusEvent], path: str | Path) -> int:
    ...
    """
    return _tcp.write_pcap(events, path, _modbus_adu)
