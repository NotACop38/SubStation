...
## ---------------------------------------------------------------------------
ci: check-python format-check lint type test schema coverage-check security ## Run the full local CI gate
	@echo "--- detection harness (Tier 1 generate->detect->report) runs under 'test'"
	@echo "--- coverage map + Navigator layer checked without rewriting the snapshot"
