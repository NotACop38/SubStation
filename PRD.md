...
- **Tier 2 — independent parser and stateful-rule checks.** Run PCAPs through Zeek/ICSNPP, in Docker by default or explicitly with native Zeek. Compare core Modbus transaction identity, address/quantity, values and exception outcomes, reparse the external Modbus corpus, compare request counts for unsupported Modbus, per-message DNP3 headers/object/control fields and modeled S7 request/response details, and execute the four Zeek rules. This does not validate every detail field, response, timing edge or device interaction. S7 requires a compiled plugin; `--require-complete` turns missing shipped checks into failures. No Suricata rules or runner are currently shipped.
...
- **Independent checks (Tier 2):** compare supported Modbus fields and responses against Zeek/ICSNPP, including attributed external captures; compare DNP3 per-message headers, IIN and object/control fields with native parser callbacks. Compare S7 COTP and modeled request/response detail fields with an explicit pinned-parser PDU-reference transform; keep unsupported Modbus count checks explicitly limited. Test stateful rules on fire/quiet PCAPs. Timing, all extensions, device behavior and SIEM backend equivalence remain unqualified.
...
|Synthetic telemetry doesn’t match real Zeek          |Detections that only work on our JSON aren’t credible                                                       |Tier-2 core Modbus fields and DNP3 message/detail comparison; S7 modeled request/response details; timing and remaining payload semantics stay explicitly limited                                                |
