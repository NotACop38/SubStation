...
- [x] Reject ambiguous or invalid JSON/rules and unused simulator parameters.
- [x] Keep X1 trust fixed after training; test recurring anomalies in native Zeek.
- [x] Repair S7 wire encoding and validate request identity/counts with the real S7 plugin.
...
- [x] Confirm `PRD.md` §7 locked decisions with the lead. (Locked and enshrined as "The 5 LOCKED decisions" in `CLAUDE.md` (PRD §7); the entire Phase 0–5 build shipped on them through the v0.1.0 release.)
...
- [x] `pyproject.toml` with pinned deps (scapy, pySigma, PyYAML; dev tooling under the `dev` extra) targeting Python 3.11+.
...
- [x] Author **detections** (deliberately mix engines to prove both rails): (M1/M2 **Sigma** + M3 **Zeek** committed under `detections/`; proves both rails per PRD §6.5.)
  - [x] **M1** Unauthorized register/coil write — Sigma. The synthetic policy scopes source, destination, port, unit, holding-register function and the complete span within 40–49. `tests/test_m1_policy.py` checks an independent interval oracle; `anomalous-m1-span-beyond-policy.yaml` now fires. Coils and writes to other services/assets remain outside the example policy.
  - [x] **M2** Illegal / abnormal function code — **Sigma**. (`detections/sigma/modbus_m2_illegal_function_code.yml`; reserved/undefined code via `action_class: other`, or an `ILLEGAL_FUNCTION`/`ILLEGAL_DATA_ADDRESS` exception. **Now fully validated Tier-1:** the emitter encodes a genuinely-undefined request code (`0x42` → Zeek `unknown-66`) + the `ILLEGAL_FUNCTION` exception, so both M2 arms fire on the anomalous scenario and stay quiet on benign. VERIFY closed — `0x09` is the legacy PROGRAM_484 in Zeek's table, not undefined. Registry status `validated`.)
...
- [x] **Local CI — Tier 1 (CLAUDE.md override: NO GitHub Actions).** `make ci` runs the full generate→detect→report loop (format, lint, strict mypy, the metadata-driven detection harness, schema validation, coverage-map + Navigator drift check, and `make security`) and fails on any contract violation; the pre-push hook (`make hooks`) gates every push. (Replaces the GitHub-Actions item per CLAUDE.md.)
- [x] **Local CI — Tier 2.** `make verify` runs Zeek/ICSNPP request identity/count comparisons and stateful fire/quiet checks. Native Zeek 8.2.2 plus ICSNPP S7comm 1.3.0 produced 41 passes, no failures and one optional Suricata skip in September 2026. Use `--require-complete`; ordinary runs can explicitly skip an unavailable S7 plugin. Full detail fidelity and the pinned Docker S7 environment remain unqualified.
- [x] Polish the **one-command demo** (`make demo` / `substation demo`): runs a benign baseline (quiet) + anomalous M1/M2 (fire) and prints real hits + the registry-driven ATT&CK-for-ICS coverage map. Tier-1 needs only Python 3.11+ and a pip install of pure-Python wheels (validated in a clean venv).
...
- [x] Finalize `docs/schema.md` as the binding contract. (FROZEN for Modbus/DNP3/S7 and declared the binding contract every emitter, detection, and test binds to; the machine-readable `substation/schema/event-log.schema.json` wins on disagreement and is enforced by `make ci`'s `schema` gate + the Tier-2 fidelity check. Bound as the v0.1.0 release contract.)
- [x] Add the **ATT&CK Navigator layer** export to the coverage builder. (Pulled forward to Phase 1 alongside the coverage generator: `substation/coverage/builder.py` `render_navigator_layer` emits an `ics-attack` Navigator layer (`docs/coverage/navigator-layer.json`) from the registry — one technique object per (technique, tactic), scored by detection count. Regenerated by `make coverage-build`; checked without modification by `make ci`.)
- [x] Enforce the **files-only invariant** in code (guard against socket sends) + assert it in tests. (Pulled forward to Phase 1: `substation/emit/guard.py` `files_only_guard` wraps all emission and makes every socket connect/transmit primitive raise `FilesOnlyViolation`; `tests/test_files_only.py` proves generation opens **no** socket and the guard blocks connect/send. Hardened in the launch pass with a **codebase-wide static** scan — `tests/test_no_raw_socket_send.py` parses every `substation/**/*.py` and forbids outbound `connect`, raw/`AF_PACKET` sockets, and scapy transmit functions — run under `make ci` and `make security`.)

**Deliverables:** green CI (both tiers); polished demo; README; Navigator layer.
**Exit criteria:** clean clone → one command → hits + coverage map on Linux/macOS with Python 3.11+ and installed dependencies (Tier 1); CI green including Tier-2 fidelity check.
...
- [x] Implement/finish **cross-protocol baseline detection X1** (Zeek; learned state + set membership over the normalized envelope). (`detections/zeek/x1_cross_protocol_baseline.zeek` — normalizes Modbus `modbus_message`, DNP3 `dnp3_application_request_header`, S7comm `s7comm_header`/`s7comm_read_szl`, and S7comm-plus `s7comm_plus_header` into one `(orig_h, resp_h, func)` tuple and runs a single learned baseline over all three protocol families. Three deviation classes in precedence order — **new talker** > **new asset pair** > **new function for a pair** — with the learned state injected via `redef`-able `known_talkers`/`known_pairs`/`known_funcs` (the Tier-2 runner derives it from the benign baselines) plus an optional self-learn window. Registry id `X1`, protocol `cross` (new registry label), ATT&CK Discovery/**T0846** (verified, same as M3/D4/S3; PRD §5.4's Lateral Movement relation noted in the doc). Benign+anomalous scenarios span protocols: all three `benign-baseline.yaml` add `X1` to `exercises.quiet`; Modbus scenarios cover new talker/pair, DNP3 covers a new function, and `scenarios/s7/anomalous-x1-new-function.yaml` proves a non-SZL S7 function reaches X1. Doc + FP profile: `detections/docs/X1-cross-protocol-baseline.md`. Tier-2 like the other Zeek rails; harness enforces contract linkage now.)
- [x] Polish the coverage map: rendered table in README/docs + downloadable Navigator layer; show covered vs gap techniques. (`substation/coverage/builder.py` now renders a **covered-vs-gap** view over the full ICS tactic matrix (`_ICS_TACTICS`, stable tactic IDs) in both `coverage.md` and `coverage.json` (`tactic_coverage`), plus a Navigator-layer download link. The single committed snapshot lives at `docs/coverage/` (`coverage.md` + `coverage.json` + `navigator-layer.json`); `make coverage-build` writes that snapshot, and `make coverage-check` is the drift gate against it. 5/12 tactics currently covered.)
- [ ] **M4 (Modbus bulk / unusual register-range reads)** — PRD §5.1 target; not yet authored. Requires VERIFY of ATT&CK technique IDs against the live matrix plus a Sigma-correlation or Zeek design before implementation; do **not** invent technique IDs. Tracked stub only.
...
- [x] Launch polish: examples/screens of the demo output and coverage map for the README. (README quick start now shows the verbatim `make demo` output — quiet-on-benign + fired M1/M2 + the registry-driven ATT&CK-for-ICS coverage map.)
...
- [x] One command, clean clone, <5 min to first success (Tier 1, Python-only). (Validated in a fresh venv: `pip install -e .` then `make demo` → hits + coverage map, exit 0. Pure-Python wheels only; no Zeek/Suricata/Docker/hardware.)
- [x] All eleven shipped detections have synthetic fire/quiet evidence, including S3 and X1 in native Zeek with S7. These results qualify the fixtures and tested engine versions, not deployment readiness or measured production effectiveness.
- [x] Every ATT&CK mapping cites a **verified** technique ID. (Re-confirmed against the live ATT&CK-for-ICS matrix 2026-06; restructured IDs T1692.001 / T1691.002 and T0888 verified current with their tactics.)
- [x] Local gates supported; current execution evidence is recorded in the September review. Native Tier 2 includes S7. Docker-image qualification remains separate.
- [x] README answers “why this exists” and shows the payoff visually.
- [x] Safety posture explicit and enforced: files-only simulator, defensive-only, passive/isolated honeypot. (Runtime files-only guard + a codebase-wide **static** no-raw-socket-send invariant test, both run under `make ci`/`make security`.)
- [x] Clear, documented path to add protocols/detections. (`docs/adding-a-protocol.md` + `docs/adding-a-detection.md`, keyed to the Detection Contract.)
- [x] **Local release pipeline** (CLAUDE.md: no cloud CI/CD). `make release` (`scripts/release/run.py`, also `/release`) is idempotent + repeatable: re-runs `make ci` + `make verify` as the gate, builds the sdist + wheel, regenerates and commits the coverage map + ATT&CK Navigator-layer snapshot (`docs/coverage/`) and the demo transcript (`docs/demo-output.txt`), bumps `pyproject.toml` + promotes `CHANGELOG.md`, and tags the release locally. **v0.1.0** cut over a green Tier-1 + Tier-2 gate.
