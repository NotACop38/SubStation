...
- [x] Author **detections** (deliberately mix engines to prove both rails): (M1/M2 **Sigma** + M3 **Zeek** committed under `detections/`; proves both rails per PRD §6.5.)
  - [x] **M1** Unauthorized register/coil write — **Sigma** (allow-list by source/unit/register). (`detections/sigma/modbus_m1_unauthorized_write.yml`; allow-list by source. Fire/quiet validated over real emitted telemetry: 0 hits on `benign-baseline`, fires on both rogue write requests in `anomalous-m1`. The unit/register-policy dimension is documented as the next increment — needs the evaluator’s numeric-range support.)
