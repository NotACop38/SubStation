...
- [x] **Local CI — Tier 1 (CLAUDE.md override: NO GitHub Actions).** `make ci` runs the full generate→detect→report loop (format, lint, strict mypy, the metadata-driven detection harness, schema validation, coverage-map + Navigator regeneration, and `make security`) and fails on any contract violation; the pre-push hook (`make hooks`) gates every push. (Replaces the GitHub-Actions item per CLAUDE.md.)
- [x] **Local CI — Tier 2 (CLAUDE.md override: NO GitHub Actions).** `make verify` (`scripts/verify/run.py`) runs containerized **Zeek + the pinned ICSNPP** analyzers to (a) run the **fidelity check** (diff our Modbus/DNP3 JSON vs real Zeek/ICSNPP output) and (b) execute the Zeek detections (M3/D4/X1) in their real engine asserting Tier-1 fire/quiet. 27 checks pass; S7 (S3/X1 S7 path) and Suricata are explicit, documented skips (no S7 C++ build toolchain in the runtime image; no Suricata rules shipped). See `docs/launch-readiness.md`.
...
- [~] All shipped detections satisfy the Detection Definition of Done. (M1, M2, D1–D3, S1–S2 fully validated Tier-1; M3, D4, X1 validated in **real Zeek** by `make verify`. **S3** is contract-complete and Tier-1-linked but its real-engine fire/quiet is **gated on the `icsnpp-s7comm` C++ plugin** — skipped where no build toolchain is available; see `docs/launch-readiness.md`.)
...
- [x] CI green (Tier 1 always; Tier 2 fidelity + Zeek/Suricata). (`make ci` green; `make verify` green — Modbus/DNP3 fidelity + M3/D4/X1 Zeek detections; S7 + Suricata are explicit documented skips.)
