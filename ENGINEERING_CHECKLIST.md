...
- [x] Generate the **first coverage-map row(s)** from detection metadata. (`substation/coverage/` — `python -m substation.coverage` renders, from `detections/registry.yaml` only, `docs/coverage/coverage.md` + `docs/coverage/coverage.json` (technique, tactic, protocol, detection ID, engine, tier, status) and `docs/coverage/navigator-layer.json`. Deterministic and generated, never hand-edited; `docs/coverage/` is the single committed snapshot. `make coverage-build` is wired into `make ci`, which regenerates it from the registry every run so it cannot drift; `make coverage-check` verifies the committed snapshot against the registry.)
